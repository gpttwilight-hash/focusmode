    ...
    function tick() {
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        if (distSq < crSq && eng > 0.01) {
          ...
          if (p.bulgeOnly) {
            const strength = 1 - dist / p.cursorRadius;
            const push = strength * strength * p.bulgeStrength * eng;
        ...
        if (p.waveAmplitude > 0) {
          drawY += Math.sin(d.ax * 0.03 + animationTime) * p.waveAmplitude;
          drawX += Math.cos(d.ay * 0.03 + animationTime * 0.7) * p.waveAmplitude * 0.5;
        }

        if (p.sparkle) {
          const hash = ((i * 2654435761) ^ (frameCount >> 3)) >>> 0;
          const size = hash % 100 < 3 ? rad * 1.8 : rad;
          activeCtx.moveTo(drawX + size, drawY);
          activeCtx.arc(drawX, drawY, size, 0, TWO_PI);
        } else {
